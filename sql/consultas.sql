...
SELECT
	...
	(
		...
		FROM
			...
			JOIN "EDUTECH".DBO.ALUNOS A ON A.ID = M.ID_ALUNO
		WHERE
			M.ID_CURSO = C.ID
			AND A.ATIVO = TRUE
			AND M.ID_STATUS <> 3
...
SELECT
	...
	COUNT(DISTINCT AL.ID) AS QTD_ALUNOS,
...
FROM
	...
	LEFT JOIN "EDUTECH".DBO.ALUNOS AL
		ON AL.ID = M.ID_ALUNO AND AL.ATIVO = TRUE AND M.ID_STATUS = 1
WHERE
	I.ATIVO = TRUE
GROUP BY
	I.NOME
